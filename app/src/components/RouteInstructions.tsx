...
export default function RouteInstructions() {
  ...
  const inclineBuckets = useMemo(() => {
    ...
    for (let i = 1; i < coords.length; i++) {
      ...
      const bucketIndex = INCLINE_BUCKETS.findIndex(
        (b) => gradePercent > b.min && gradePercent <= b.max,
      );
      if (bucketIndex !== -1) totals[bucketIndex] += dist;
    }
    const totalDist = totals.reduce((a, b) => a + b, 0);
    if (totalDist === 0) return null;
    return INCLINE_BUCKETS.map((b, i) => ({
      ...b,
      meters: totals[i],
      pct: (totals[i] / totalDist) * 100,
    }));
  }, [path]);
          ...
          <div className="flex h-2 overflow-hidden rounded-full">
            {inclineBuckets.map((b) => (
              <div
                ...
                title={`${b.label}: ${Math.round(b.meters)} m`}
              />
            ))}
          </div>
        </div>
