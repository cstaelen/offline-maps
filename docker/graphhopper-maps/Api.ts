...
export class ApiImpl implements Api {
    ...
    async geocode(
        ...
        // Photon (self-hosted) speaks a different query dialect than the GraphHopper Directions API
        // this app was built for: no `key`/`provider`, `lang` instead of `locale`, `lat`/`lon` instead of `point`.
        ...
        if (additionalOptions) {
            for (const key in additionalOptions) {
                if (key === 'point') {
                    const [lat, lon] = additionalOptions[key].split(',')
        ...
        if (response.ok) {
            // Photon returns GeoJSON, not the { hits, took } shape of the GraphHopper Directions API
            const geojson = await response.json()
            const hits: GeocodingHit[] = (geojson.features ?? []).map((f: any) => ({
