    ...
    private routeIfReady(state: QueryStoreState, zoom: boolean): QueryStoreState {
        if (QueryStore.isReadyToRoute(state)) {
            ...
            if (state.customModelEnabled) {
                ...
                } else if (maxDistance < 3_000_000) {
                    // Force no alternatives for longer custom model routes.
                    // Raised from the upstream 700km limit: our GraphHopper instance has profiles_lm enabled,
                    // which (unlike CH) stays compatible with custom models and keeps long routes fast enough.
                    requests = [
